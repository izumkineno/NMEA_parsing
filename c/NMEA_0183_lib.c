 ...
 * @return 1：检测到，0：未检测到
 */
_Bool check_header(uint8_t buf, uint8_t *header, uint8_t header_len, uint8_t *state, Type type) {
 ...
 */
int check_body(uint8_t *buffer, uint8_t *vec_data[], uint8_t vec_data_len, int vec_data_size,
               uint8_t *header, uint8_t header_len, uint8_t *state, Type type,
               _Bool *body_flag, uint16_t *last_coma_index, uint8_t *coma_counter) {
 ...
 * @return 浮点数的真实数据
 */
double get_lat_lon(uint8_t *data_raw, _Bool is_lat) {
    uint8_t *t1, *t2;
    if (is_lat) {
